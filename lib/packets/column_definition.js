...
//
/**
 * @getter {string} catalog
 * @getter {string} schema
 * @getter {string} table
 * @getter {string} orgTable
 * @getter {string} orgName
 * @property {string} name
 * @property {number} columnLength
 * @property {number} columnType
 * @property {number} characterSet
...
class ColumnDefinition {
  constructor(packet, clientEncoding) {
    ...
    this.encoding = CharsetToEncoding[this.characterSet] || 'binary';
    this.name = PacketParser.decode(
  ...
  // node-mysql compatibility: alias "columnLength" to "length"
  get length() {
    return this.columnLength;
  }

  // node-mysql compatibility: look up type as string on demand
  get type() {
    return typeNames[this.columnType];
  }
}
...
const addString = function(name) {
  Object.defineProperty(ColumnDefinition.prototype, name, {
    get: function() {
      ...
      const value = PacketParser.decode(
        this._buf.slice(start, end),
        this.encoding === 'binary' ? this._clientEncoding : this.encoding
      );

      // Cache decoded string
